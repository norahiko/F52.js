...
function Reloader(callback) {
    ...
    this.wss = null;
    this.version = "0";
    this.host = f52._getHost();
    this.port = f52._getPort();
    this.root = "http://" + this.host + ":" + this.port;
...
Reloader.prototype._requestHandler = function(req, res) {
    var urlinfo = urlModule.parse(req.url);
...
Reloader.prototype._serveClientScript = function(res) {
    ...
    src = src.replace("<HOST>", f52._getHost());
    src = src.replace("<PORT>", f52._getPort());
